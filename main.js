...
(function () {
  ...
  function initRCMap() {
    if (rcMap) return;
    rcMap = L.map('rcMap', { zoomControl: true, scrollWheelZoom: false })
             .setView([11.0, 78.5], 6);
  ...
  async function getOSRMRoute(from, to) {
    ...
    return null; // will use haversine fallback
  }
  ...
  function haversineKm(from, to) {
    ...
    const a = Math.sin(dLat/2)**2 +
              ...
              Math.sin(dLon/2)**2;
    return (R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)) * 1.3).toFixed(1); // ×1.3 road factor
  }
  ...
  function dot(color) {
    return L.divIcon({
      className: '',
      html: `<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2.5px solid #fff;box-shadow:0 0 6px rgba(0,0,0,.5)"></div>`,
  ...
  async function calculateRoute() {
    ...
    try {
      ...
      // 2. Route (OSRM or fallback)
      let routeData = await getOSRMRoute(from, to);
      ...
      if (!routeData) {
        ...
        routeData = {
          distanceKm: km,
          durationMin: Math.round((parseFloat(km) / avgSpeedKmH) * 60),
          geometry: null
        };
      }
      ...
      const baseFare   = Math.round(actualDist * activeRate);
      const total      = baseFare + activeBata;
      const displayDist = actualDist.toFixed(1);
      const displayDur  = formatDur(routeData.durationMin * (isRT ? 2 : 1));

      // 4. Update distance / time
      document.getElementById('rcDistance').textContent = displayDist + ' km' + (usedFallback ? ' ~' : '');
      document.getElementById('rcTime').textContent = displayDur + (usedFallback ? ' ~' : '');

      // 5. Update fare breakdown
      document.getElementById('rcRate').textContent   = inr(activeRate) + '/km';
      document.getElementById('rcDistLabel').textContent = `(${displayDist} km${isRT ? ' RT' : ''})`;
      ...
      rcPickupMarker = L.marker([from.lat, from.lon], { icon: dot('#22c55e') })
        .bindPopup(`<b>Pickup:</b> ${pickup}`).addTo(rcMap);
      rcDropMarker = L.marker([to.lat, to.lon], { icon: dot('#ef4444') })
        .bindPopup(`<b>Drop:</b> ${drop}`).addTo(rcMap);

      if (routeData.geometry) {
        rcRouteLayer = L.geoJSON(routeData.geometry, {
          style: { color: '#F5B800', weight: 5, opacity: 0.85 }
        }).addTo(rcMap);
        rcMap.fitBounds(rcRouteLayer.getBounds(), { padding: [30,30] });
      } else {
        // fallback: draw straight line + fit
        rcRouteLayer = L.polyline([[from.lat,from.lon],[to.lat,to.lon]], {
          color:'#F5B800', weight:4, dashArray:'10 8', opacity:0.7
        }).addTo(rcMap);
      ...
      bookBtn.onclick = function () {
        ...
        const msg = encodeURIComponent(
          ...
          `Route: ${pickup} → ${drop}\n` +
          `Trip Type: ${isRT ? 'Round Trip' : 'One Way'}\n` +
          `Vehicle: ${activeVehicle}\n` +
          `Distance: ${displayDist} km\n` +
      ...
      alert('Could not calculate route.\nPlease check the location names.\n\nError: ' + err.message);
    } finally {
      btn.disabled = false;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (section && 'IntersectionObserver' in window) {
      const obs = new IntersectionObserver(entries => {
        if (entries[0].isIntersecting) { initRCMap(); obs.disconnect(); }
      }, { threshold: 0.1 });
      obs.observe(section);
    } else { initRCMap(); }
    ...
    function setupRCAutocomplete(inputId, listId) {
      ...
      input.addEventListener('input', function () {
        ...
        const matches = LOCATIONS.filter(l =>
          l.a.toLowerCase().includes(q) || l.c.toLowerCase().includes(q)
        ).slice(0, 8);
...
(function () {
  ...
  function initRCMap(){if(rcMap)return;rcMap=L.map('rcMap',{zoomControl:true,scrollWheelZoom:false}).setView([11.0,78.5],6);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© <a href="https://openstreetmap.org">OpenStreetMap</a>',maxZoom:18}).addTo(rcMap);}
  async function geocode(q){const res=await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q+', India')}&limit=1&accept-language=en`);const d=await res.json();if(!d.length)throw new Error('Location not found: '+q);return{lat:parseFloat(d[0].lat),lon:parseFloat(d[0].lon)};}
  async function getSuggestions(q){try{const res=await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q+', India')}&limit=6&accept-language=en&addressdetails=1`);return await res.json();}catch(_){return[];}}
  async function getOSRMRoute(a,b){const urls=[`https://router.project-osrm.org/route/v1/driving/${a.lon},${a.lat};${b.lon},${b.lat}?overview=full&geometries=geojson`,`https://routing.openstreetmap.de/routed-car/route/v1/driving/${a.lon},${a.lat};${b.lon},${b.lat}?overview=full&geometries=geojson`];for(const url of urls){try{const ctrl=new AbortController();const t=setTimeout(()=>ctrl.abort(),8000);const res=await fetch(url,{signal:ctrl.signal});clearTimeout(t);const d=await res.json();if(d.routes&&d.routes.length){const r=d.routes[0];return{distanceKm:(r.distance/1000).toFixed(1),durationMin:Math.round(r.duration/60),geometry:r.geometry};}}catch(_){}}return null;}
  function haversineKm(a,b){const R=6371,dLat=(b.lat-a.lat)*Math.PI/180,dLon=(b.lon-a.lon)*Math.PI/180;const h=Math.sin(dLat/2)**2+Math.cos(a.lat*Math.PI/180)*Math.cos(b.lat*Math.PI/180)*Math.sin(dLon/2)**2;return(R*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h))*1.3).toFixed(1);}
  function fmtDur(m){const h=Math.floor(m/60),r=m%60;return h?`${h} hr${h>1?'s':''} ${r?r+' min':''}`.trim():`${r} min`;}
  function inr(n){return '\u20B9'+Math.round(n).toLocaleString('en-IN');}
  function dot(color){return L.divIcon({className:'',iconSize:[14,14],iconAnchor:[7,7],html:`<div style="width:14px;height:14px;border-radius:50%;background:${color};border:2.5px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.5)"></div>`});}
  function setupNominatimAC(inputId,listId){
    ...
    input.addEventListener('input',function(){
      ...
      const local=LOCATIONS.filter(l=>l.a.toLowerCase().includes(lower)||l.c.toLowerCase().includes(lower)).slice(0,6).map(l=>({display_name:`${l.a},${l.c},${l.s}`,address:{city:l.a,state:l.s}}));
  ...
  async function calculateRoute(){
    const pickup=document.getElementById('rcPickup').value.trim(),drop=document.getElementById('rcDrop').value.trim();
    const isRT=document.querySelector('input[name="rcTrip"]:checked').value==='roundtrip';
    if(!pickup||!drop){alert('Please enter both pickup and drop locations.');return;}
    const btn=document.getElementById('rcCalcBtn');btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Calculating...';
    try{
      initRCMap();
      const[from,to]=await Promise.all([geocode(pickup),geocode(drop)]);
      ...
      if(route.geometry){rcRouteLayer=L.geoJSON(route.geometry,{style:{color:'#F5B800',weight:5,opacity:0.85}}).addTo(rcMap);rcMap.fitBounds(rcRouteLayer.getBounds(),{padding:[30,30]});}
      else{rcRouteLayer=L.polyline([[from.lat,from.lon],[to.lat,to.lon]],{color:'#F5B800',weight:4,dashArray:'10 8',opacity:0.7}).addTo(rcMap);rcMap.fitBounds([[from.lat,from.lon],[to.lat,to.lon]],{padding:[40,40]});}
      setTimeout(()=>rcMap.invalidateSize(),100);
      const addMsg=document.getElementById('rcAddMsg');
    ...
    }catch(err){alert('Could not calculate route. Please check location names.\n\n'+err.message);}
    finally{btn.disabled=false;btn.innerHTML='<i class="fas fa-route"></i> Calculate Route';}
  }
  document.addEventListener('DOMContentLoaded',function(){
    document.querySelectorAll('.rc-vtab').forEach(btn=>{btn.addEventListener('click',function(){document.querySelectorAll('.rc-vtab').forEach(b=>b.classList.remove('active'));this.classList.add('active');activeRate=parseInt(this.dataset.rate);activeBase=parseInt(this.dataset.base);activeBata=parseInt(this.dataset.bata);activeVehicle=this.dataset.vehicle.charAt(0).toUpperCase()+this.dataset.vehicle.slice(1);});});
    const calcBtn=document.getElementById('rcCalcBtn');if(calcBtn)calcBtn.addEventListener('click',calculateRoute);
    const section=document.getElementById('route-calculator');
    if(section&&'IntersectionObserver'in window){const obs=new IntersectionObserver(e=>{if(e[0].isIntersecting){initRCMap();obs.disconnect();}},{threshold:0.1});obs.observe(section);}else{initRCMap();}
    setupNominatimAC('rcPickup','rcPickupList');
    setupNominatimAC('rcDrop','rcDropList');
    setupNominatimAC('pickupLoc','pickupList');
    setupNominatimAC('dropLoc','dropList');
  });
})();
